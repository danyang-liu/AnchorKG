...
class Recommender(BaseModel):
    ...
    def __init__(self, config, doc_feature_embedding, entity_embedding, relation_embedding, entity_adj, relation_adj):
    ...
    def get_neighbors(self, entities):
        ...
        return torch.tensor(neighbor_entities).cuda(), torch.tensor(neighbor_relations).cuda()
    ...
    def get_anchor_graph_embedding(self, anchor_graph):
        ...
        anchor_graph_nodes_embedding = self.entity_embedding(torch.tensor(anchor_graph_nodes).cuda())
        ...
        neibor_relations_embedding = self.relation_embedding(neibor_relations)
        ...
        anchor_embedding_weight = self.softmax(self.anchor_weights_layer2(self.elu(self.anchor_weights_layer1(anchor_embedding))))
